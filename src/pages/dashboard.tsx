...
import { AllegationListPage as ShowCaseList } from 'pages/allegations';

function DashboardPage() {
  const { session } = useSession();

  const perpetratorStat = useSWR<PaginatedInterface<PerpetratorInterface>>(
    () => `/perpetrators`,
    () => getPerpetrators({ limit: 1, offset: 0 }),
  );

  const investigatorStat = useSWR<PaginatedInterface<InvestigatorInterface>>(
    () => `/investigators`,
    () => getInvestigators({ limit: 1, offset: 0 }),
  );

  const victimStat = useSWR<PaginatedInterface<VictimInterface>>(
    () => `/victims`,
    () => getVictims({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Perpetrators',
      result: perpetratorStat?.data?.totalCount,
      isLoading: perpetratorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Investigators',
      result: investigatorStat?.data?.totalCount,
      isLoading: investigatorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Victims',
      result: victimStat?.data?.totalCount,
      isLoading: victimStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`allegations`} passHref legacyBehavior>
                  <Button
